...
from .dependencies.mediapipe.mp_utils import LMKExtractor
...
class Cropper(object):
    ...
    def extract_lkm_from_landmarks(self, img_rgb_: np.ndarray, src_face):
        lmks = []
        for index in range(len(src_face)):
            x = src_face[index].x * img_rgb_.shape[1]
            y = src_face[index].y * img_rgb_.shape[0]
            lmks.append([x, y])
    ...
    def crop_source_image(self, img_rgb_: np.ndarray, crop_cfg: CropConfig):
        ...
        if self.image_type == "human_face":
            src_face = self.lmk_extractor(img_rgb)
    ...
    def calc_lmk_from_cropped_image(self, img_rgb_, **kwargs):
        src_face = self.lmk_extractor(img_rgb_)
        if src_face is None or len(src_face) == 0:
            log("No face detected in the source image.")
            return None
        elif len(src_face) > 1:
        ...
        src_face = src_face[0]
        lmk = self.extract_lkm_from_landmarks(img_rgb_, src_face)
    ...
    def crop_source_video(self, source_rgb_lst, crop_cfg: CropConfig, **kwargs):
        ...
        for idx, frame_rgb in enumerate(source_rgb_lst):
            if idx == 0 or trajectory.start == -1:
                src_face = self.lmk_extractor(frame_rgb)
                if src_face is None or len(src_face) == 0:
    ...
    def calc_lmks_from_cropped_video(self, driving_rgb_crop_lst, **kwargs):
        ...
        for idx, frame_rgb_crop in enumerate(driving_rgb_crop_lst):
            if idx == 0 or trajectory.start == -1:
                src_face = self.lmk_extractor(frame_rgb_crop)
                if src_face is None or len(src_face) == 0:
                    log(f"No face detected in the frame #{idx}")
                ...
                src_face = src_face[0]
                lmk = self.extract_lkm_from_landmarks(frame_rgb_crop, src_face)
