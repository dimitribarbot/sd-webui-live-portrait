...
from ...utils import load_file_from_url, load_state_dict_from_url
...
class BlazeFaceDetector(FaceDetector):
    def __init__(self, device, back_model=False, path_to_detector=None, path_to_anchor=None, verbose=False,
        ...
        if path_to_detector is None:
            if back_model:
                model_weights = load_state_dict_from_url(models_urls['blazeface_back_weights'], model_dir)
                model_anchors = np.load(load_file_from_url(models_urls['blazeface_back_anchors'], model_dir))
            else:
                model_weights = load_state_dict_from_url(models_urls['blazeface_weights'], model_dir)
                model_anchors = np.load(load_file_from_url(models_urls['blazeface_anchors'], model_dir))
        else:
            model_weights = unsafe_torch_load(path_to_detector)
